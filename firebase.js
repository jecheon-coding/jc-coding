...
// --- 3. Public Review Functions (With Edit/Delete Menu) ---
    ...
    try {
        ...
        if (docSnap.exists() && docSnap.data().password === password) {
            if (action === 'delete') {
                ...
                const newContent = prompt('수정할 내용을 입력하세요.', docSnap.data().content);
                if (newContent) {
...
window.addEventListener('click', (e) => {
    if(!e.target.classList.contains('review-menu-btn')) {
        document.querySelectorAll('.review-dropdown').forEach(m => m.classList.remove('active'));
    }
});

// --- 4. Admin Management (Keep full features) ---
...
async function loadAdminReviews() {
    ...
    snap.forEach(d => {
        ...
        div.style.padding = '1rem'; div.style.marginBottom = '1rem'; div.style.display = 'flex'; div.style.justifyContent = 'space-between';
        div.innerHTML = `<div><strong>${escapeHTML(d_data.name)}</strong><p>${escapeHTML(d_data.content)}</p></div><button class="nav-cta" style="background:#ef4444; width:auto; padding:0 1rem;" onclick="adminDeleteItem('reviews', '${id}')">삭제</button>`;
...
async function loadAdminInquiries() {
    ...
    snap.forEach(d => {
        ...
        table.innerHTML += `<tr style="border-bottom:1px solid rgba(255,255,255,0.05);"><td style="padding:15px;">${date}</td><td style="padding:15px;"><strong>${escapeHTML(d_data.name)}</strong></td><td style="padding:15px;">${escapeHTML(d_data.phone)}</td><td style="padding:15px;">${escapeHTML(d_data.grade)}</td><td style="padding:15px; max-width:300px;">${escapeHTML(d_data.message)}</td></tr>`;
    });
}
...
async function loadAdminSchedules() {
    ...
    snap.forEach(d => {
        ...
        div.innerHTML = `<span class="section-tag">${escapeHTML(d_data.category)}</span><p><strong>${escapeHTML(d_data.course)}</strong> - ${escapeHTML(d_data.status)}</p>`;
        list.appendChild(div);
    });
}
